...
![Опис зображення](Images/Lab2/Lab2_Image35.png)
...
------------------------------------------------------------------------------------------------
У SystemVerilog інструкція `assign` з'єднує один сигнал із іншим, як у прикладі з кодом:

```verilog
assign led_o = switch_i;
```

Ця інструкція призначає значення сигналу `switch_i` до `led_o` безпосередньо, тобто об'єднує їх у режимі простого передавання сигналу. Це приклад *комбінаторного зв'язку*, де вихід `led_o` миттєво реагує на будь-які зміни у вхідному `switch_i`.

Інструкція `assign` зазвичай використовується для створення постійних зв'язків у коді, коли потрібно, щоб один сигнал безперервно передавав значення іншому.
...
![Опис зображення](Images/Lab2/Lab2_Image36.png)


---------------------------------------------------------

У цьому модулі регістр, що зберігає вхід попереднього такту годинника, реалізований за допомогою змінної `prev_inp_q`, яка оновлюється на кожному позитивному фронті сигналу `clk_i`:
...
- **`always_ff @(posedge clk_i)`** визначає, що блок буде виконуватися на кожному передньому фронті (зростанні) сигналу `clk_i`.
- **`prev_inp_q <= prev_inp_d;`** у блоку `always_ff` зберігає поточне значення `inp_i` (через `prev_inp_d`) у `prev_inp_q`, таким чином зберігаючи вхідний сигнал від попереднього такту.

### Інструкція, що визначає вихід

Вихід `pulse_o` визначено за допомогою оператора `assign`:

```verilog
assign pulse_o = (inp_i && !prev_inp_q);
```

Цей оператор перевіряє, чи відбувся перехід з `0` до `1` на вході `inp_i` між двома тактами годинника. Якщо `inp_i` є `1`, а попереднє значення `prev_inp_q` було `0`, то `pulse_o` стає `1` на один такт, сигналізуючи про виникнення переднього фронту.
...
![Опис зображення](Images/Lab2/Lab2_Image39.png)



---------------------------------------------------------------------------------------------------------------------
Цей модуль `zybo_top` є верхньорівневим дизайном для плати Zybo FPGA, що управляє відображенням стану входів (перемикачів і кнопок) на світлодіодах. Він має два режими роботи, що перемикаються за допомогою кнопки. Ось детальний огляд функціональних блоків:

### 1. **Параметри і порти**
   - **Параметри** `NumLeds` та `NumBtns` задають кількість світлодіодів та кнопок відповідно.
   - **Порти**
     - `clk_i` — вхідний тактовий сигнал.
     - `btn_i` — вектор кнопок.
     - `switch_i` — вектор перемикачів.
     - `led_o` — вектор світлодіодів.

### 2. **Сигнали і логіка для кнопок**
   - **`btn` та `dbnc_btn`** — вектори для обробки сигналів кнопок (включаючи подавлення дребезгу та виявлення фронту).
   - Блок `generate` виконує наступні дії для кожної кнопки:
     - **`debouncer`** пригнічує дребезг кнопок, використовуючи параметр `NumCycles`.
     - **`rising_edge_pulser`** видає імпульс на один такт для кожного натискання кнопки.

### 3. **Модулі `led_switcher` і `led_toggler`**
   - **`led_switcher`** безпосередньо передає стан перемикачів `switch_i` на світлодіоди `led_switcher_oup`.
   - **`led_toggler`** дозволяє користувачу циклічно змінювати стан світлодіодів, натискаючи кнопки `next`, `prev` і `toggle`.

### 4. **Перемикання режимів за допомогою кнопки**
   - За допомогою кнопки `btn[0]` вибирається активний режим, який визначає, який вихід буде передаватися на світлодіоди:
     - **`mode_d` і `mode_q`** визначають поточний і наступний режим, який переключається між `led_switcher_oup` і `led_toggler_oup`.

### 5. **Мультиплексор для вибору активного виходу світлодіодів**
   - У блоці `always_comb` за допомогою оператора `case` вибирається вихід `led_o` в залежності від поточного режиму `mode_q`:
     - Якщо `mode_q` = 0, світлодіоди відображають `led_switcher_oup`.
     - Якщо `mode_q` = 1, відображається `led_toggler_oup`.

### 6. **Генератор скидання**
   - **`rst_gen`** генерує сигнал скидання `rst_n` після певної кількості тактів (заданих параметром `RstClkCycles`).

### 7. **Регістри для збереження режиму**
   - **`always_ff @(posedge clk_i)`** оновлює стан режиму `mode_q` на кожному передньому фронті тактового сигналу, зберігаючи його значення з `mode_d`.
   - При скиданні (`~rst_n` = 0) режим встановлюється на `0`.

### Загалом, модуль `zybo_top`:
- Контролює стан світлодіодів на основі режиму, вибраного за допомогою кнопок і перемикачів.
- Забезпечує пригнічення дребезгу кнопок і відображає дії користувача через світлодіоди в різних режимах роботи.
...
### З’єднання сигналів у цьому модулі
У цьому коді переважно використовуються **інструкції присвоєння** `assign` та **порти** для з'єднання сигналів. Ось основні підходи, які застосовуються в цій реалізації:
1. **`assign` для безпосереднього з'єднання сигналів**: Наприклад, `assign prev_inp_d = inp_i;` використовується для підключення одного сигналу до іншого.
2. **Порти модулів** для передачі сигналів між різними частинами дизайну:
   - Сигнали кнопок `btn_i` передаються в модулі `debouncer` та `rising_edge_pulser` через відповідні порти.
   - Світлодіодні виходи `led_switcher_oup` та `led_toggler_oup` підключаються до виходу `led_o` через `always_comb`.

Загалом, **більшість сигналів з'єднані за допомогою модулів та їхніх портів**.

### Значення затримки для `debouncers`
Затримка для `debouncer` встановлена через параметр `NumCycles`, який має значення **5,000,000** тактів. Це число тактів визначає тривалість подавлення дребезгу для кнопок. Чи відповідає ця затримка очікуванням, залежить від частоти тактового сигналу `clk_i`:
- Припустимо, що тактова частота, наприклад, становить 100 МГц, тоді 5,000,000 тактів відповідають 50 мс.
- Це досить стандартне значення для подавлення дребезгу і може відповідати очікуванням, якщо система має швидку тактову частоту.

Отже, значення затримки відповідає типовим вимогам для стабілізації кнопок у більшості цифрових систем.
...
![Опис зображення](Images/Lab2/Lab2_Image37.png)

![Опис зображення](Images/Lab2/Lab2_Image38.png)
...
![Опис зображення](Images/Lab2/Lab2_Image32.png)
